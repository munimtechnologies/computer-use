...
**Let any coding agent use your computer.** An open-source [MCP](https://modelcontextprotocol.io) server that reads the screen through accessibility trees, clicks and types *in the background* so your mouse stays yours, shows an agent pointer where it is working, zooms in on small text, and drives tabs in your signed-in Chrome — on **macOS, Windows and Linux**.
